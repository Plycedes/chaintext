...
contract Chaintext{
    ...
    function addFriend(address friend_key, string calldata name) external{
        ...
        require(checkAlreadyFriends(msg.sender, friend_key) == false, "Already a friend");
    ...
    function checkAlreadyFriends(address pubkey1, address pubkey2) internal view returns(bool){
        if(users[pubkey1].friendList.length > users[pubkey2].friendList.length){
            address tmp = pubkey1;
            pubkey1 = pubkey2;
            pubkey2 = tmp;
        }
        for(uint256 i = 0; i < users[pubkey1].friendList.length; i++){
            if(users[pubkey1].friendList[i].pubkey == pubkey2)return true;
        }
        return false;
    }
    ...
    function _addFriend(address me, address friend_key, string memory name) internal {
        ...
        users[me].friendList.push(newFriend);
    }
